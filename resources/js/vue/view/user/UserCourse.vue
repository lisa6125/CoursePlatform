<template>
    <div class="text-2xl text-gray-800 course">
        <div class="course-setting card my-5 mx-auto p-1">
                <span class="fs-2 m-3 ms-5">我參加的課程</span>
            <div class="course-lists p-3 d-flex flex-wrap justify-content-center align-items-center">
                <div class="item p-2 me-3 mb-3" v-for="item,idx in userCourse" :key="item.id">
                    <div class="pic">
                        <img :src="item.pic1"/>
                    </div>
        ...
        <!-- 彈跳視窗 -->
        <div v-show="showCard" class="modal fade" ref="vuemodal" :v-model="nowOpen" id="exampleModal" tabindex="-1">
...
export default {
  ...
  computed:{
    ...
    methods:{
        ...
        openModal(num){
            this.nowOpen = this.userCourse[num];
        ...
        cancelParticipateCourse(id){
            axios.post('/api/user/UserCancelParticipate/'+id,{userId:this.user.id})
            .then((res)=>{
                new this.$swal({
                    icon: 'success',
                    title: res.data,
